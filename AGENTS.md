...
- Write every changelog entry unslopped: plain concrete words, short sentences, no AI tells. Apply the `unslop` skill before writing.
- Never write `Fixed` for behavior that has not been released yet. Fold unreleased work into `Added` or `Changed` instead. Reserve `Fixed` for bugs in a shipped release.
